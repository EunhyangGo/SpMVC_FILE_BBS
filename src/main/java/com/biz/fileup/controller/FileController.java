...
import com.biz.fileup.service.FileUpService;
...
public class FileController {
	 ...
	 * @Controller에서
	 * 
	 * @ResponseBody가 설정된 method가 String(문자열)을
	 * return하면 문자열 자체가 client에게 전송되어
	 * web browser에게 문자열이 찍히게 된다.
	 * 
	 * @ResponseBody가 설정이 안된 method가 
	 * String(문자열)을 return하면
	 * WEB-INF/views/문자열.jsp파일을 찾아 읽어서
	 * 랜더링 한 후, client에게 전송한다.
	 * 
	 * 또한
	 * @RestController로 설정을 하게되면
	 * 모든 method가 자동으로 @ResponseBody로 설정이 되어버린다.
	 * 
	 * 그리고
	 * jackson-bind를 Dependency에 추가해 주게 되면
	 * @ResponseBody는 String뿐만아니라
	 * List나 사용자가 직접 생성한 클래스 type을 return할 수 있고
	 * 해당하는 내용은 적절하게 변환되어 json형태로 
	 * client에게 전송된다.
	 *    
	 */
